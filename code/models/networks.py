...
class SIGGRAPHGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, norm_layer=nn.BatchNorm2d, use_tanh=True, classification=True):
        ...
        # regression output
        model_out = [nn.Conv2d(128, output_nc, kernel_size=1, padding=0, dilation=1, stride=1, bias=use_bias), ]
        if(use_tanh):
            model_out += [nn.Tanh()]
    ...
    def forward(self, input_values, input_mask):
        #print("Note: ", input_A.shape, input_B.shape, mask_B.shape)
        tmp = torch.cat((input_values, input_mask), dim=1)
        conv1_2 = self.model1(tmp)
