...
def build_mask(shape, R=70, G=20):
    rand_vals = torch.randint(0, 100, size=(shape[0],shape[2],shape[3]))

    mask_R = (rand_vals >= 0) & (rand_vals < R)
    mask_G = (rand_vals >= R) & (rand_vals < (R + G))
    mask_B = (rand_vals >= (R + G)) & (rand_vals <= 100)
    final_mask = torch.stack([mask_R, mask_G, mask_B], axis=1)

    return final_mask.float()


def reveal_rgb_values(data_raw, mask, opt):
    data = {}
    data_raw[0] = data_raw[0]
    data['V'] = (data_raw[0]*mask).sum(axis=1) # BATCH X dim=1 X H X W
    data['V'] = data['V'].unsqueeze(1)
    data['MASK_V'] = mask # set mask
    data['REAL_RGB'] = data_raw[0] # RGB
...
def get_colorization_data(data_raw, opt, ab_thresh=5., p=.125, num_points=None):
    ...
    data['A'] = data_lab[:,[0,],:,:] # luminance!!

    # TODO: rgb to matrix of values
    data['B'] = data_lab[:,1:,:,:] # ab

    # mask_B
    # set mask according to values in B
...
def calculate_mse_np(img1, img2):
    import numpy as np
    SE_map = (1.*img1-img2)**2
    cur_MSE = np.mean(SE_map)
    return cur_MSE
